 ...
 */
function unblockcn_redirector(details) {
    return {redirectUrl: 'https://github.com/Unblocker/Unblock-Youku/issues/468'};
};

function setup_unblockcn_redirector() {
    if (!chrome.webRequest.onBeforeRequest.hasListener(unblockcn_redirector)) {
        chrome.webRequest.onBeforeRequest.addListener(
            unblockcn_redirector,
            {
                urls: [
                    'http://unblockcn.com/*',
                    'http://*.unblockcn.com/*',
        ...
        console.log('unblockcn_redirector is set');
    } else {
        var err_msg = 'unblockcn_redirector is already there!';
        console.error(err_msg);
        ga_report_error('Unexpected Error', err_msg);
    }
}
